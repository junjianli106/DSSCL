...
parser.add_argument('--exp-dir', default='experiment_wo_he', type=str,
...
parser.add_argument('--weight_orig', default=0.5, type=float,
                    help='There is no need to use in this code')
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch>=args.warmup_epoch:
            ...
            for num_cluster in args.num_cluster:
                ...
                cluster_result['silhouette'].append(torch.zeros(len(eval_dataset),dtype=torch.long).cuda())
...
def train(train_loader, model, criterion, optimizer, epoch, args, cluster_result=None):
    ...
    for i, (images, index) in enumerate(train_loader):
        ...
        output, target = model(im_q=images[0], im_k=images[1], cluster_result=cluster_result, index=index)
