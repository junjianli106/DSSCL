...
# 定义数据集路径
dataset_path = '/home/junjianli/data/pcam'

# 修改为新的保存目录
mid_fix = 'Stain-Separation'
stage='train'
PATCH_PATH = os.path.join(dataset_path, stage)  # 输入图像目录
OUTPUT_BASE_PATH = os.path.join(dataset_path, mid_fix, stage)  # 输出图像基目录
...
def get_img_list(path, k=None):
    ...
    """
    # 使用glob递归查找所有.png文件
    img_list = glob.glob(os.path.join(path, '**', '*.jpg'), recursive=True)
...
def main(img):
    ...
    try:
        ...
        name: neg/29668.jpg
        '''
        class_name = os.path.dirname(img)
